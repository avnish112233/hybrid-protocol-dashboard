...
export function ProfileHeader() {
  ...
  const athleteName = profile?.vi?.profile.name ?? profile?.name ?? "Hybrid Protocol";
  const lastTested  = formatDate(profile?.vi?.dexa?.date);
  ...
  return (
    ...
    >
      <div
        ...
        style={{ backgroundImage: "repeating-linear-gradient(135deg, var(--hyrox-yellow) 0 14px, transparent 14px 32px)" }}
      />
      <div
      ...
      <div className="relative flex items-start justify-between">
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.32em]" style={{ color: "var(--hyrox-yellow)" }}>
            Vital Insights
          </div>
          <h1
            ...
            style={{
              ...
              fontSize: athleteName.length > 12 ? "1.75rem" : "1.875rem",
              maxWidth: "14ch",
            }}
          >
            {athleteName}<span style={{ color: "var(--hyrox-yellow)" }}>.</span>
          </h1>
          <div
        ...
        <Link
          ...
          style={{ borderColor: "color-mix(in oklab, var(--hyrox-yellow) 40%, transparent)", color: "var(--hyrox-yellow)" }}
